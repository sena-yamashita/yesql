if Code.ensure_loaded?(Exqlite) do
  ...
  defmodule Yesql.Stream.SQLiteStream do
    ...
    defp build_indexed_query(table, index_column, start_value, end_value) do
      ...
      where_clauses =
        if start_value,
          do: ["#{index_column} >= '#{start_value}'"] ++ where_clauses,
          else: where_clauses

      where_clauses =
        if end_value,
          do: ["#{index_column} <= '#{end_value}'"] ++ where_clauses,
          else: where_clauses

      if where_clauses != [] do
        base_query <>
          " WHERE " <> Enum.join(where_clauses, " AND ") <> " ORDER BY #{index_column}"
      else
      ...
      case Exqlite.Sqlite3.prepare(conn, explain_sql) do
        {:ok, statement} ->
          ...
          if not uses_index do
            IO.warn(
              "Query does not use index. Consider creating an index for better performance."
            )
          end
