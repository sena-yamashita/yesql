defmodule Yesql.Unit.CastSyntaxTest do
  ...
  describe "各データベースのキャスト構文サポート" do
    test "PostgreSQL :: キャスト構文の変換" do
      ...
      # 複数のキャスト
      {sql, _} =
        Yesql.Driver.convert_params(driver, "SELECT :name::text, :age::integer, :data::jsonb", [])

      assert sql == "SELECT $1::text, $2::integer, $3::jsonb"

      # 配列キャスト
      {sql, _} =
        Yesql.Driver.convert_params(driver, "SELECT :tags::text[], :numbers::integer[]", [])
    ...
    test "DuckDB :: キャスト構文の変換" do
      ...
      # 複雑な型
      {sql, _} =
        Yesql.Driver.convert_params(driver, "SELECT :date::DATE, :time::TIME, :json::JSON", [])
    ...
    test "MySQL CAST関数形式" do
      ...
      # MySQLはCAST関数を使用
      {sql, _} = Yesql.Driver.convert_params(driver, "SELECT CAST(:value AS CHAR)", [])
      assert sql == "SELECT CAST(? AS CHAR)"

      # 複数のCAST
      {sql, _} =
        Yesql.Driver.convert_params(
          driver,
    ...
    test "SQLite CAST関数形式" do
      ...
      # SQLiteもCAST関数を使用
      {sql, _} = Yesql.Driver.convert_params(driver, "SELECT CAST(:value AS INTEGER)", [])
  ...
  describe "エッジケース" do
    test "文字列内の :: は変換されない" do
      ...
      {sql, _} =
        Yesql.Driver.convert_params(driver, "SELECT 'value::text' as example, :real::integer", [])
