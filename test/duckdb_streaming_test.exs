defmodule DuckDBStreamingTest do
  ...
  setup_all do
    case System.get_env("DUCKDB_TEST") do
      "true" ->
        ...
        # 実際に挿入された行数を確認
        {:ok, count_result} = Duckdbex.query(conn, "SELECT COUNT(*) FROM streaming_test", [])
        [[count]] = Duckdbex.fetch_all(count_result)
        IO.puts("テストデータ挿入数: #{count}")
  ...
  describe "並列スキャン" do
    ...
    test "テーブルの並列スキャン", %{conn: conn} do
      # 注意: create_parallel_scanは未実装のため、スキップ
      {:ok, stream} =
        Yesql.Stream.DuckDBStream.create_parallel_scan(
          conn,
          "streaming_test",
          parallelism: 4,
          chunk_size: 100
  ...
  describe "Parquetエクスポート" do
    ...
    test "Parquetファイルへのエクスポート", %{conn: conn, tmp_dir: tmp_dir} do
      ...
      result =
        Yesql.Stream.DuckDBStream.export_to_parquet(
          conn,
          sql,
          [],
          file_path,
  ...
  describe "ウィンドウ付きストリーミング" do
    ...
    test "時系列データのウィンドウ処理", %{conn: conn} do
      ...
      result =
        Yesql.Stream.DuckDBStream.create_windowed_stream(
          conn,
  ...
  describe "パフォーマンステスト" do
    ...
    test "大規模データのストリーミング性能", %{conn: conn} do
      # まず実際のデータ数を確認
      {:ok, count_result} = Duckdbex.query(conn, "SELECT COUNT(*) FROM streaming_test", [])
      [[actual_count]] = Duckdbex.fetch_all(count_result)
      IO.puts("実際のデータ数: #{actual_count}")
      ...
      {:ok, stream} =
        Yesql.Stream.DuckDBStream.create(
          conn,
          sql,
          [],
          chunk_size: 1000,
