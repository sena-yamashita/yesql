defmodule BatchTest do
  ...
  describe "バッチクエリ実行" do
    test "複数クエリの成功実行", %{conn: conn, driver: driver} do
      ...
      # 結果を確認
      {:ok, %{rows: rows}} = Postgrex.query(conn, "SELECT name, value FROM batch_test ORDER BY id", [])
    ...
    test "トランザクション内でのエラー処理", %{conn: conn, driver: driver} do
      ...
      queries = [
        ...
        {"INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Invalid", "not_a_number"]},  # エラー
        {"INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Never", 300]}
      ]
      
      {:error, _reason, partial_results} = Batch.execute(queries, 
        driver: driver, 
        conn: conn,
        transaction: true
      )
  ...
  describe "名前付きバッチクエリ" do
    ...
    test "名前付きクエリのエラー処理", %{conn: conn, driver: driver} do
      named_queries = %{
        ...
        invalid: {"INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Invalid", "not_a_number"]},
        never_executed: {"INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Never", 300]}
      }
      
      {:error, _reason, partial_results} = Batch.execute_named(named_queries, 
        driver: driver, 
        conn: conn
      )
  ...
  describe "パイプライン実行" do
    test "前の結果を使った連鎖実行", %{conn: conn, driver: driver} do
      ...
      pipeline = [
        # 最初のクエリ
        fn _ ->
          {"INSERT INTO batch_test (name, value) VALUES ($1, $2) RETURNING id, name", ["Pipeline1", 100]}
        end,
        
        # 前の結果のIDを使う
        fn [%{id: prev_id}] ->
          {"INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Pipeline2_ref_#{prev_id}", 200]}
        end,
    ...
    test "パイプラインでのエラー処理", %{conn: conn, driver: driver} do
      pipeline = [
        ...
        fn _ ->
          {"SELECT COUNT(*) FROM batch_test", []}  # 実行されない
        end
      ]
  ...
  describe "トランザクション管理の改善" do
    test "分離レベルの設定", %{conn: conn, driver: driver} do
      result = Transaction.transaction(conn, fn conn ->
        # トランザクション内でクエリを実行
        Postgrex.query!(conn, "INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Isolated", 100])
        :ok
      end, driver: driver, isolation_level: :serializable)
    ...
    test "明示的なロールバック", %{conn: conn, driver: driver} do
      ...
      result = Transaction.transaction(conn, fn conn ->
        Postgrex.query!(conn, "INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Rollback", 100])
    ...
    test "セーブポイントの使用", %{conn: conn, driver: driver} do
      ...
      result = Transaction.transaction(conn, fn conn ->
        # 最初の挿入
        Postgrex.query!(conn, "INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["First", 100])
        
        # セーブポイント作成
        {:ok, _} = Transaction.savepoint(conn, "sp1", driver: driver)
        
        # 2番目の挿入
        Postgrex.query!(conn, "INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Second", 200])
        
        # セーブポイントまでロールバック
        {:ok, _} = Transaction.rollback_to_savepoint(conn, "sp1", driver: driver)
        
        # 3番目の挿入
        Postgrex.query!(conn, "INSERT INTO batch_test (name, value) VALUES ($1, $2)", ["Third", 300])
