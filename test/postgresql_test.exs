defmodule PostgreSQLTest do
  ...
  describe "PostgreSQL専用機能のテスト" do
    ...
    test "範囲型のサポート", %{postgrex: conn} do
      ...
      {:ok, _} = Postgrex.query(conn, """
        ...
        ($3, $4)
      """, [
        %Postgrex.Range{lower: 100, upper: 200, lower_inclusive: true, upper_inclusive: false},
        %Postgrex.Range{lower: ~D[2024-01-01], upper: ~D[2024-12-31], lower_inclusive: true, upper_inclusive: true},
        %Postgrex.Range{lower: 50, upper: 150, lower_inclusive: true, upper_inclusive: true},
        %Postgrex.Range{lower: ~D[2024-06-01], upper: ~D[2024-08-31], lower_inclusive: true, upper_inclusive: true}
      ])
    ...
    test "全文検索機能", %{postgrex: conn} do
      ...
      {:ok, _} = Postgrex.query(conn, """
        ...
        ($5, $6)
      """, [
        "Elixir Programming", "Elixir is a dynamic, functional language",
        "Phoenix Framework", "Phoenix is a productive web framework for Elixir",
        "PostgreSQL Database", "PostgreSQL is a powerful open source database"
      ])
    ...
    test "ウィンドウ関数のサポート", %{postgrex: conn} do
      ...
      assert product_a_rows |> List.last |> Enum.at(3) == Decimal.new(370) # 100 + 150 + 120
    end

    test "トランザクション分離レベルのテスト", %{postgrex: conn} do
      # READ COMMITTEDレベルでのトランザクション
      {:ok, _} = Postgrex.transaction(conn, fn conn ->
        {:ok, _} = Postgrex.query(conn, "INSERT INTO test_isolation (value) VALUES ($1)", [100])
        {:ok, result} = Postgrex.query(conn, "SELECT value FROM test_isolation WHERE value = $1", [100])
        assert length(result.rows) == 1
      end)
    ...
    test "部分インデックスの効果", %{postgrex: conn} do
      ...
      {:ok, explain} = Postgrex.query(conn, """
    ...
    test "プリペアドステートメントのテスト", %{postgrex: conn} do
      # プリペアドステートメントの準備
      {:ok, query} = Postgrex.prepare(conn, "get_by_status", """
  ...
  describe "PostgreSQLストリーミング機能" do
    ...
    test "カーソルを使ったストリーミング", %{postgrex: conn} do
      # トランザクション内でカーソルを使用
      {:ok, count} = Postgrex.transaction(conn, fn conn ->
        # カーソルの宣言
        {:ok, _} = Postgrex.query(conn, """
  ...
  defp truncate_postgres_test_tables(ctx) do
    ...
    tables = [
      "test_jsonb", "test_arrays", "test_uuid", "test_timestamps",
      "test_enum", "test_ranges", "test_fulltext", "test_hierarchy",
      "test_sales", "test_isolation", "test_partial_index", "test_aggregates"
    ]
