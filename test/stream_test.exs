defmodule StreamTest do
  ...
  setup_all do
    ...
    # PostgreSQL
    connections =
      if System.get_env("POSTGRESQL_STREAM_TEST") == "true" do
        {:ok, pg_conn} =
          Postgrex.start_link(
            hostname: "localhost",
            username: "postgres",
    ...
    # MySQL
    connections =
      if System.get_env("MYSQL_STREAM_TEST") == "true" do
        {:ok, mysql_conn} =
          MyXQL.start_link(
            hostname: "localhost",
            username: "root",
    ...
    # SQLite
    connections =
      if System.get_env("SQLITE_STREAM_TEST") == "true" do
    ...
    # DuckDB
    connections =
      if System.get_env("DUCKDB_STREAM_TEST") == "true" do
    ...
    # MSSQL
    connections =
      if System.get_env("MSSQL_STREAM_TEST") == "true" do
        {:ok, mssql_conn} =
          Tds.start_link(
            hostname: "localhost",
            username: "sa",
    ...
    # Oracle
    connections =
      if System.get_env("ORACLE_STREAM_TEST") == "true" do
        {:ok, oracle_conn} =
  ...
  describe "基本的なストリーミング" do
    test "大量データのストリーミング取得", %{connections: connections} do
      for {driver, conn} <- connections do
        ...
        # ストリーミング実行
        {:ok, stream} =
          Stream.query(get_conn(conn), sql, [5000],
            driver: driver,
            chunk_size: 100
          )
    ...
    test "チャンクサイズの動作確認", %{connections: connections} do
      for {driver, conn} <- connections do
        ...
        # 小さなチャンクサイズでストリーミング
        {:ok, stream} =
          Stream.query(get_conn(conn), sql, [100],
            driver: driver,
            chunk_size: 10
          )
  ...
  describe "エラーハンドリング" do
    test "無効なSQL", %{connections: connections} do
      for {driver, conn} <- connections do
        result = Stream.query(get_conn(conn), "INVALID SQL", [], driver: driver)
    ...
    test "接続エラー", %{connections: connections} do
      for {driver, _conn} <- connections do
        ...
        result = Stream.query(invalid_conn, "SELECT 1", [], driver: driver)
  ...
  describe "ドライバー固有機能" do
    ...
    test "MySQL: サーバーサイドカーソル", %{connections: connections} do
      if conn = connections[:mysql] do
        ...
        {:ok, stream} =
          MySQLStream.create_with_cursor(
            conn,
    ...
    test "SQLite: FTS5ストリーミング", %{connections: connections} do
      if conn = connections[:sqlite] do
        ...
        {:ok, stream} =
          SQLiteStream.create_fts_stream(
            conn,
            "documents",
            "number",
            chunk_size: 10
  ...
  defp setup_postgresql(conn) do
    ...
    # 大量のテストデータを挿入
    insert_test_data_postgresql(conn, 100_000)
  end
  ...
  defp setup_mysql(conn) do
    ...
    insert_test_data_mysql(conn, 100_000)
  end
  ...
  defp setup_sqlite(conn) do
    ...
    insert_test_data_sqlite(conn, 100_000)
  end
  ...
  defp setup_duckdb(conn) do
    ...
    insert_test_data_duckdb(conn, 100_000)
  end
  ...
  defp setup_mssql(conn) do
    ...
    insert_test_data_mssql(conn, 100_000)
  end
  ...
  defp setup_oracle(conn) do
    ...
    insert_test_data_oracle(conn, 100_000)
  end
  ...
  # DuckDB
  defp get_conn({_db, conn}), do: conn
  defp get_conn(conn), do: conn

  defp get_simple_select_sql(:postgrex),
    do: "SELECT * FROM stream_test WHERE id <= $1 ORDER BY id"
