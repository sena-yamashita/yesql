defmodule EctoStreamingTest do
  ...
  describe "基本的なストリーミング" do
    ...
    test "max_rowsオプションの動作" do
      ...
      {:ok, stream} = Yesql.Stream.EctoStream.create(TestRepo, sql, params, max_rows: 20)
  ...
  describe "トランザクション内でのストリーミング" do
    test "transaction_streamの実行" do
      ...
      {:ok, count} =
        Yesql.Stream.EctoStream.transaction_stream(
          TestRepo,
          sql,
          params,
          fn row ->
    ...
    test "バッチ処理の実行" do
      ...
      {:ok, batch_count} =
        Yesql.Stream.EctoStream.batch_process(
          TestRepo,
          sql,
          params,
          10,
  ...
  describe "並列ストリーミング" do
    ...
    test "parallel_streamの動作" do
      ...
      {:ok, stream} =
        Yesql.Stream.EctoStream.parallel_stream(
          TestRepo,
          sql,
          params,
          parallelism: 4,
