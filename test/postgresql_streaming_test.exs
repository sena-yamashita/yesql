defmodule PostgreSQLStreamingTest do
  ...
  defp fetch_cursor_recursive(conn, cursor_name, chunk_size, acc) do
    {:ok, result} = Postgrex.query(conn, "FETCH #{chunk_size} FROM #{cursor_name}", [])
    ...
    case result.rows do
      ...
      rows ->
        new_rows =
          Enum.map(rows, fn [id, value, created_at] ->
